...
function EditUser() {
  ...
  const getUserdetails = (id) => UsersService.details(id);
  ...
  const { data: clientData } = useSWR('LIST_CLIENTS', () => clientService.getClients());
  ...
  const { data: rolesdata } = useSWR('LIST_ROLES', () => roleservice.getRoles());
  ...
  const [activeStatus, setActiveStatus] = useState(eachclicntdata?.IsActive ? 'active' : 'inactive');
 ...
 useEffect(() => {
  ...
  const onSubmit = (data) => {
    ...
    UsersService.edit(id, backendFormat)
    .then((res) => {
  ...
  return (
    <div className='overflow-auto text-black mt-4 p-3'>
      <div className='text-black' style={{ fontSize: '16px', fontWeight: '600' }}>
        User Management
      </div>

      <div className='d-flex justify-content-between'>
        <div className='text-black' style={{ fontSize: '25px', fontWeight: '600' }}>
          Edit User
        </div>
        <div className='d-flex gap-1'>
          <Button onClick={() => router.back()} color='white'>
           ...
           <Button onClick={editMode ? handleSaveClick : handleToggleEditMode} color='primary' className='px-4'>
        ...
        <Form className='mt-2' onSubmit={handleSubmit(onSubmit)}>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <>
                      ...
                      />
                      {errors.lastname && (
                        <div className='text-danger'>{String(errors.lastname.message)}</div>
                      )}
                    </>
                  )}
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <>
                      ...
                      />
                      {errors.firstname && (
                        <div className='text-danger'>{String(errors.firstname.message)}</div>
                      )}
                    </>
                  )}
         ...
         <Col xl='4'>
          ...
          <div className="d-flex flex-column mt-2">
            <Label className="text-black" style={{ fontSize: '16px', fontWeight: '400' }}>
              Status
            </Label>
            <div className="d-flex gap-1">
              <div className="d-flex gap-1">
